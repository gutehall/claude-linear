...
- **Shipped / duplicate / in flight** → **SKIP** — never reimplement and never auto-close. Comment with what you found, then move it out of the queue for a human (leave `auto-claude` in place so they can re-queue):
  ```bash
  linear issue update <id> --status "Backlog" --label "auto-claude" --label "needs-human"
  ```
  Print `autopilot: <id> appears already solved (PR #188 / ISSUE-29) — flagged needs-human, skipping.` Continue loop; do not implement.
...
Follow the **ship-gate** skill in **autonomous** mode. Unfixable test/build or unfixable Critical/High → **STOP LOOP**, leave issue In Progress, print `autopilot: <id> failed self-review — <finding>. Stopping loop.`
...
Each cycle ships its own PR and merges with `--delete-branch`, so the branch is gone after merge. **Project mode:** the next cycle re-creates `<project-slug>-YYYY-MM-DD` fresh from the updated `<base>` (step 3). Do **not** reuse or re-push the merged branch — its commits are already squashed onto `<base>`, so reuse would diverge.
