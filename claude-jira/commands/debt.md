...
/debt --changed    # Only files changed since the last full /debt scan
```

## Flow

Follow the **debt** skill for the full scan, prioritization, and issue-creation procedure. Scope the scan to `$ARGUMENTS` if a path was given, otherwise scan the full codebase. Honor `--changed` as defined in the skill.
