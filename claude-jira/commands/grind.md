...
Follow the **ship-gate** skill in **autonomous** mode. Unfixable test/build or unfixable Critical/High → **STOP LOOP**, leave issue In Progress, print `grind: <key> failed self-review — <finding>. Stopping loop.`
...
Each cycle ships its own PR and merges with `--delete-branch`, so the branch is gone after merge. **Project mode:** the next cycle re-creates `<epic-slug>-YYYY-MM-DD` fresh from the updated `<base>` (step 3). Do **not** try to reuse or re-push the merged branch — its commits are already squashed onto `<base>`, so reuse would diverge.
