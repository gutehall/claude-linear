---
name: whatchanged
description: Show everything that has changed since the last time this command was run — git commits, files, Jira issues, and PRs. Persists a checkpoint so each run only shows new changes. Use this skill whenever the user runs /whatchanged or asks what has changed since the last run.
---
...
### Jira issues updated since last run
...
If `jira` is unavailable or returns an error: skip silently and note "Jira data unavailable" at the bottom.
...
- If there are Jira issues in Done: suggest `/retro` to reflect on shipped work
...
- Do not create any Jira issues or PRs from this command — read-only only
